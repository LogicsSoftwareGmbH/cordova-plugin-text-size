...
Cordova plugin, **iOS only**: reports the system text size (Dynamic Type — *Settings › Display & Brightness › Text Size*) as a percentage of the default size, once and on every change. The app applies it to its WebView content itself.
...
Nothing is stored — the OS setting is the single source of truth, exactly like on Android. Android is deliberately not part of this plugin.
